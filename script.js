...
let isPasswordValid = () => {
  /* password should be atleast 8 characters long .Shoukld contain atleast 1 number, 1 special symbol, 1 lower case and 1 upper case*/
  const passwordRegex =
   /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/
...
submitBtn.addEventListener("mouseover", ()=>{
    ...
    if(!isUsernameValid() || !isPasswordValid()){
        ...
        //if the button on the left hand side. move it to the right hand side
        if(offset <= 100){
